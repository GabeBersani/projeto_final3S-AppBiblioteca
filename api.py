...
from models import Livros, Usuarios, Emprestimos, db_session
...
def cadastrar_emprestimo():
    ...
    try:
        ...
        usuario = db_session.query(Usuarios).filter_by(id_usuario=int(dados['id_usuario'])).first()
        livro = db_session.query(Livros).filter_by(id_livro=int(dados['id_livro'])).first()
...
def editar_usuario(id):
    ...
    try:
        ...
        usuario_editado = db_session.execute(
            select(Usuarios).where(Usuarios.id_usuario == int(id))
        ).scalar()
        ...
        if usuario_editado.CPF != cpf:
            cpf_existe = db_session.execute(
                select(Usuarios).where(Usuarios.CPF == cpf)
            ).scalar()
...
def livro_status():
    try:
        livros = db_session.execute(select(Livros)).scalars().all()
...
def editar_status_livro(id):
    ...
    livro = db_session.execute(select(Livros).where(Livros.id_livro == id)).scalar()
